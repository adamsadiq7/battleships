...
void dx(State *s, int operand){
  s->previousdx = s->dx;
  s->dx = s->dx + operand;
}

void dy(State *s, int operand, display *d){
  s->previousdy = s->dy;
  s->dy = s->dy + operand;

  if (s->pen == true){
    line(d,s->previousdx, s->previousdy, s->dx, s->dy);
  }
  s->previousdx = s->dx;
}

void dt(State *s, int operand){

}

void pen(State *s, int operand){
  if (s->pen == false){
    s->pen = true;
  }
  else{
...
void unpack(char holdin, int opcode, int operand, State *s, display *d, FILE *in){

    opcode = ((holdin >> 6) & 0x03);
    operand = ((holdin) & 0x3F);



    int unsign = ((holdin) & 0x3F);
    if (operand >= 32){
      operand = operand - 64;
    }
...
void interpret(FILE *in, display *d) {
  State sdata = {0,0,0,0,false,false};
  State *s = &sdata;
  unsigned char holdin;

  int opcode = 0;
  int operand = 0;
  printf("Calling Unpack Function..\n");
  holdin = fgetc(in);

  while (!feof(in)){
    unpack(holdin, opcode, operand, s, d, in);
    holdin = fgetc(in);
  }
  printf("Closing File..\n");
}
